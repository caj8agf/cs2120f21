...
begin
  ...
  cases r_bij with r_surj r_inj,
  cases r_inj with r_tot r_oto,
  cases r_surj with r_tot r_all,
  unfold total_function at r_tot,
  cases r_tot with r_fun alldef,
  unfold function at r_fun,
  unfold single_valued at r_fun,
...
begin
  ...
  cases inj with tot oto,
  unfold function,
  unfold single_valued,
  assume b a1 a2 irba1 irba2,
  unfold inverse at irba1 irba2,
  apply oto irba1 irba2,
end
...
begin 
  ...
  cases r_bij with r_surj r_inj,
  cases s_bij with s_surj s_inj,

  unfold surjective at r_surj s_inj,
  unfold injective at r_inj s_inj,

  cases r_surj with r_tot r_all,
  cases s_surj with s_tot s_all,
  cases r_inj with r_tot r_oto,
  cases s_inj with s_tot s_oto,

  unfold total_function at r_tot s_tot,
  unfold function at r_tot s_tot,
  unfold single_valued at r_tot s_tot,
  have r_sv := r_tot.left,
  have s_sv := s_tot.left,
  have r_def := r_tot.right,
  have s_def := s_tot.right,
  unfold defined at r_def s_def,
  ...
  --injective
    ...
    --total_function
      ...
      --defined
        ...
        exact and.intro h1 h,
    --oto
      assume a1 a2 c cra1c cra2c,
      unfold composition at cra1c cra2c,
      cases cra1c with b1 cra1c,
      cases cra2c with b2 cra2c,
      have sb1c := cra1c.left,
      have sb2c := cra2c.left,
      have ra1b1 := cra1c.right,
      have ra2b2 := cra2c.right,
      have bb : b1 = b2 := s_oto sb1c sb2c,
      rw bb at ra1b1,
      apply r_oto ra1b1 ra2b2,
end
