...
/-
Proof: First we'll assume that T is any type and objects x, y, z
and w are of that type. What remains to be shown is that w = z → 
z = w. To prove this, we'll assume the premise, that w = z, and
in this context we prove z = w. By the theorem that equality is
symmetric and using the fact that w = z and the axiom of
substitutability of equals, we can rewrite w in the goal as a z, yielding
z = z as our new proof goal. But this is true by the axiom of
...
/-
Assume an arbitrary value of x and prove P x. P x must be true
